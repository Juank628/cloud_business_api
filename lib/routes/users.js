...
/****************get all users***************/
usersRoutes.get("/list", validateRole("business_administrator"), (req, res) => {
      ...
      if (err) {
        ...
        cognitoIdentityService.adminGetUser(params, function(err, data) {
          if (err) {
            res.status(400).json({
              error:
                "El susario fue creado pero no es posible obtener sus datos"
            });
          } else {
            res.json(data);
          }
...
usersRoutes.put(
  ...
  (req, res) => {
    ...
    cognitoIdentityService.adminUpdateUserAttributes(params, function(
      err,
      data
    ) {
      if (err) {
