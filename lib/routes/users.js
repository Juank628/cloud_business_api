...
const cognitoIdentityService = new Aws.CognitoIdentityServiceProvider({apiVersion: '2016-04-19', region: pool_region});
...
usersRoutes.get("/list", (req, res) => {
  cognitoIdentityService.listUsers({
    UserPoolId: process.env.USER_POOL_ID,
    AttributesToGet: ["given_name","family_name","email","custom:role","custom:locations"]
  }, (err, data) => {
    if (!err) {
