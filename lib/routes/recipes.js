...
recipesRoutes.get("/list", async (req, res) => {
  const recipes = await Recipe.find().populate("supplies").populate("family");
  res.json(recipes);
});

/*************create recipe************/
recipesRoutes.post("/create", validateRole("business_administrator"), async (req, res) => {
  const {
    name,
    supplies,
    req_supp_quantities,
...
/**************update recipe**************/
recipesRoutes.put("/update", validateRole("business_administrator"), async (req, res) => {
  const {
    id,
    name,
    supplies,
